...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    sendBuff[0] = reg | 2;
    sendBuff[1] = val;
    int check = XSpi_Transfer(&SpiInstance, sendBuff, recBuff, 2);
    if(check != 0){
        xil_printf("ERROR");
    }
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    int check = XSpi_Transfer(&SpiInstance, sendBuff, recBuff, nbytes + 1);
    if(check != 0){
        xil_printf("ERROR");
    }
    XSpi_SetSlaveSelect(&SpiInstance, 0);
...
void MAX3421E_reset(void) {
	Status = XGpio_Initialize(&Gpio_rst, XPAR_AXI_GPIO_3_DEVICE_ID);
	XGpio_SetDataDirection(&Gpio_rst, 1, 0); //configure reset, and set reset to output
	Status = XGpio_Initialize(&Gpio_int, XPAR_AXI_GPIO_2_DEVICE_ID);
...
void MAX3421E_init(void) {
	...
	//start USB timer
	Status = XTmrCtr_Initialize(&Usb_timer, XPAR_SPI_USB_DEVICE_ID);
	if (Status != XST_SUCCESS) {
